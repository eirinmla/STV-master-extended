...
class AtlFormula(Formula):
    ...
    def __str__(self):
        return "<<" + (", ".join(self.agents)) + ">>" + str(self.temporalOperator.value) + super().__str__()
...
class CtlFormula(Formula):
    ...
    def __str__(self):
        return str(self.pathQuantifier.value) + str(self.temporalOperator.value) + super().__str__()


class UclFormula(Formula):
    agents = []
    upgrades = []
    upgradeType = None

    def __str__(self):
        return "{" +  str(self.upgrades) + self.upgradeType.value + "}" + "<<" + (", ".join(self.agents)) + ">>" + super().__str__()
    ...
    def __getValue(self, item, varValues):
        if isinstance(item, str):
            if item in varValues:
                ...
                return item
        elif isinstance(item, UpgradeExpression):
            return item.evaluate(varValues)
        return item
    
    def evaluate(self, varValues):
        expression = self.__getValue(self.expression, varValues)
        if "[" in expression:  # upgrade
            # return self.__parseUpgrades()
            pass
        elif "<<" in expression: # formula with coalition
            # return self.__parseAgents(), self.__parseFormulaExpression()
            pass
        else: # formula
            # return self.__parseFormulaExpression()
            pass
...
class FormulaParser(Parser):
    ...
    def parseUclFormula(self, formulaStr):
        self.setStr(formulaStr)

        formula = UclFormula()
        formula.upgradeType = self.__parseUpgradeType()
        formula.agents = self.__parseFormulaAgents()
        formula.upgrades = self.__parseUpgrades()
    ...
    def __parseUpgradeType(self):
        ...
        if c == "+":
            ...
            raise Exception("Unknown UpgradeType")

    def __parseUpgrades(self):
        expression = []
        self.consume("{")
        while True: 
            res = self.readUntil(["[", ":", "]", "(", ")", "}"])
        ...
        if "[" in expression:  # upgrade
            ...
            return self.__parseFormulaAgents(), self.__parseFormulaExpression()
        else: # formula
            return self.__parseFormulaExpression()
